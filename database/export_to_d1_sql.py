"""
Dumps league_history.sqlite into one .sql file per year (managers get their
own small file) so each can be pushed to Cloudflare D1 with
...
def insert_statements(conn, table, columns, where=None, params=()):
    ...
    return hl.insert_or_replace_sql(table, columns, rows)
...
def main():
    ...
    managers_stmts = insert_statements(conn, "managers", hl.MANAGER_COLUMNS)
    ...
    for year in years:
        stmts = []
        stmts += insert_statements(conn, "teams", hl.TEAM_COLUMNS, where="year = ?", params=(year,))
        stmts += insert_statements(conn, "matchups", hl.MATCHUP_COLUMNS, where="year = ?", params=(year,))
        stmts += insert_statements(conn, "roster_entries", hl.ROSTER_COLUMNS, where="year = ?", params=(year,))
        stmts += insert_statements(conn, "draft_picks", hl.DRAFT_COLUMNS, where="year = ?", params=(year,))
        stmts += insert_statements(conn, "league_settings", hl.LEAGUE_SETTINGS_COLUMNS, where="year = ?", params=(year,))
